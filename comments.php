...
<div id="comentarios" class="comentarios-area">
    ...
    <?php if ( have_comments() ) : ?>
        <h2 class="comentarios-titulo" style="font-size:1.8rem;margin-bottom:var(--espaco-2xl);">
            ...
            printf( _n( '%1$s Comentário em "%2$s"', '%1$s Comentários em "%2$s"', $count, 'temaaventuras' ),
...
<?php
// Callback personalizado para renderizar cada comentário
...
function tema_aventuras_comment( $comment, $args, $depth ) {
    ...
    <<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>" <?php comment_class('comentario-item', $comment); ?>
        ...
        <?php if ( '0' === $comment->comment_approved ) : ?>
            <p style="color:var(--texto-muted);font-style:italic;font-size:var(--tamanho-pequeno);"><?php _e('Seu comentário está aguardando moderação.','temaaventuras'); ?></p>
        <?php endif; ?>
        <div class="comment-content wp-content"><?php comment_text(); ?></div>
        <div style="margin-top:var(--espaco-sm);">
            <?php comment_reply_link(array_merge($args,['reply_text'=>'↩ '.__('Responder','temaaventuras'),'depth'=>$depth,'max_depth'=>$args['max_depth']])); ?>
        </div>
    </<?php echo $tag; ?>>
    <?php
}
